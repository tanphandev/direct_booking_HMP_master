...
const SidebarSearch = () => {
  ...
  const page_title: cp_title[] | [] =
    footer_navigation.find((page: footer_navigation) => page.page_slug === 'english_test') || [];
  ...
  // let AmenityQuantity = 0;
  let selectedAmenities=payload||[];
  ...
  return (
    <>
      ...
      >
        ...
        <span className="text-white flex-auto uppercase  align-middle px-3  ">
          {t('SEARCH.SEARCH_RESULT_PAGE.AMENITIES')} {selectedAmenities.length===0?"":`(${selectedAmenities.length})`}
        </span>
      </button>
      <div>
      ...
      <div>
        <div className="rounded-md px-4 py-2 text-white bg-[#636363]">
          <span> {t('SEARCH.SEARCH_RESULT_PAGE.FAQ')}</span>
        </div>
        {cp_paragraphs && (
          <div className="pt-5">
            {cp_paragraphs?.map(
            ...
            <button
              onClick={GotoFaqsPage}
              className="uppercase border border-[#e4e4e4] bg-[#f5f5f5] w-full  py-2 rounded-md hover:opacity-90 hover:shadow-sm"
            >
        ...
        <div className="pt-5">
          <button
            ...
            className=" border border-[#0a7cff]  w-full mb-4 py-2 rounded-md hover:opacity-90 hover:shadow-sm"
          >
